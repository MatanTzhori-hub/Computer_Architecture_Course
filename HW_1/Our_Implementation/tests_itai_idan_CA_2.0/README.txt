...
Please read the following instructions in order to run the tests and compare your result to ours.
	...
	2. open the terminal in that directory and run the following command:
		let i=1; while((i<100)); do ./bp_main tests_itai_idan_CA_2.0/example${i}.trc > tests_itai_idan_CA_2.0/your_output_for_example${i}.out; let i++; done
	3. compare your result to our output for the tests using the following command:
		let i=1; while((i<100)); do diff -s -q tests_itai_idan_CA_2.0/example${i}.out tests_itai_idan_CA_2.0/your_output_for_example${i}.out; let i++; done
	4. Celebrate!
